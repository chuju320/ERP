...
class Action:
	...
	def find_element(self, loc):
		...
		try:
			...
			print u"BasePage中查找成功"
			return self.driver.find_element(*loc)
		except:
			print u"BasePage中查找失败"
			#self.saveScreenshot(self.driver,'unknowloc')
			print loc
	...
	def open(self,base_url, pagetitle):
		self._open(base_url, pagetitle)

	def close(self,num):
		#print '写入位置:%s %s'%(nrow,num)
		#passOrNot.passOrNot(nrow,num)
		self.driver.quit()
	...
	def getcelldata(self,filepath,sheetname, RowNum, ColNum):
		...
		ColNum:列号 从0开始
		"""
		table = self.setTable(filepath=filepath,sheetname=sheetname)
		celldata = table.cell_value(RowNum, ColNum)
		return celldata

	#读取元素定位标签和元素唯一标识
	def locate(self, index, filepath="dataEngine\\erpdata.xls", sheetname="element"):
		"""
		filepath: 文件路径
		sheetno：Sheet编号
		index: 元素编号
	...
	def Locatetext(self, index, filepath="dataEngine\\erpdata.xls", sheetname="element"):
		"""	读元素标识

		filepath: 文件路径
		sheetno：Sheet编号
		index: 元素编号
		返回值内容为criteria的值：如 "username"
		"""
		table = self.setTable(filepath, sheetname)
		for i in range(1, table.nrows):
			if index in table.row_values(i):
				return table.row_values(i)[2]
