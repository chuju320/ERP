		...
		for j in stepdata:
			if txt[1] == j[0]:
				...
				# 	exeKeyword.verifyLogin(loc_1, loc_2)
					...
					password = j[5]
					VerifyNo = j[6]
					userinput = base.locate(j[7])
					passinput = base.locate(j[8])
					Verifyimg = base.locate(j[9])
					btnsubmit = base.locate(j[10])
					self.driver = self.driver
					exeKeyword.login(username,password,VerifyNo,userinput,passinput,Verifyimg,btnsubmit)
				elif j[3] == "DBAssert":
					print j[2]
					host = '192.168.1.7'
					user = 'testerp'
